...
#define ADXML345_READMB_CMD(reg) (ADXL345_CMD_READ | ADXCL345_CMD_MULTB | (reg & 0x3F))
...
#define ACT_TAP_STATUS 0x2B // R Source of tap/double ta
...
// FIFO_CTL Bits
#define FIFO_MODE(x) (((x) & 0x3) << 6) // Question?
...
// Macros to do SPI opertaions
#define AC_READ(ac, reg) ((ac)->bops->read((ac)->dev, reg))
#define AC_WRITW(ac, reg, val) ((ac)->bops->write((ac)->dev, reg, val))
...
// Set intial adxl345 register values
static const struct adxl345_platform_data adxl_default_init = {
    .tap_threshold = 50,
    .tap_duration = 3,
...
static void adxl345_get_triple(struct adxl345 *ac, struct axis_triple *axis)
{
    ...
    axis->x = ac->saved.x;
    ac->saved.y = sign_extend32(le16_add_cpu(buf[1]), 12);
    axis->y = ac->saved.y;
    ac->saved.z = sign_extend32(le16_add_cpu(buf[2], 12));
    axis.z = ac->saved.z;
}
...
// Function called in the ISR when there is a SIGNLE_TAP event
...
static irqreturn_t adxl345_irp(int irp, void *handle)
{
    ...
    struct adxl345_platform_data *pdata = &ac->data;
...
static ssize_t adxl345_rate_show(struct dev *dev,
                                  ...
                                  const char *buf, size_t count)
{
    ...
    ACPI_WRITE(ac, BW_RATE, ac->pdata.data_rate | (ac->pdata.low_power_mode ? LOW_POWER : 0));
...
static const struct attribute_group adxl_attr_group = {
...
struct adxl345 *adxl345_probe(struct device *dev, const struct adxl345_bus_ops *bops)
{
    ...
    ac = devm_kalloc(dev, sizeof(*ac), GFP_KERNEL);
    if (!ac)
    {
    ...
    pdata = *ac->pdata;            // change where pdata points, now to pdata in pricate ac

    ac->input = input_dev;
    ac->dev = dev; // dev is &spi->dev
    // Store the SPI operations in your private struct
    ac->bops = bops;
    ...
    // Attach the input device and the private struct
    ...
    // Request threadeed interrupt

    err = devm_request_threaded_irq(input_dev->dev.parent,
                                    ac->irq, NULL, adxl345_irq, IRQF_TRIGGER_HIGH | IRQF_ONESHOT,
                                    dev_name(de), ac);
    ...
    /*
     * Set the data rate and power mode (higher noise, less power)
     */
    ...
    // Return initialized private struct
    return ac;
...
// initialize struct adxl345_bus_ops to SPI bus function
...
static int adxl345_spi_probe(struct spi_device *spi) // Question: why we have 3 probe
{
    struct adxl345 *ac;
    ...
    // Attach the SPI device to the private struct
...
static int adxl345_spi_remove(struct spi_device *spi)
{
    ...
    dev_info(ac->dev, "unregisterd accelerometer\n");
    return 0;
}
