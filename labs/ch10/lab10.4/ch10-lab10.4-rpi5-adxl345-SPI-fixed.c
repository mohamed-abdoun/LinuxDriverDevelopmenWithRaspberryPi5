...
#define ADXML345_READMB_CMD(reg) (ADXL345_CMD_READ | ADXL345_CMD_MULTB | (reg & 0x3F))
...
struct adxl345 *adxl345_probe(struct device *dev, const struct adxl345_bus_ops *bops)
{
    ...
    err = devm_request_threaded_irq(input_dev->dev.parent,
                                    ac->irq, NULL, adxl345_irp, IRQF_TRIGGER_HIGH | IRQF_ONESHOT,
                                    dev_name(dev), ac);
...
static struct spi_driver adxl345_driver = {
    ...
    .remove = (void *)adxl345_spi_remove,
    .id_table = adxl345_id,
};
