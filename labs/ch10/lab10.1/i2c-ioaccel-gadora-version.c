...
/* Poll function */
static void ioaccel_poll(struct input_dev *input)
{
	...
	if ( (val > 0xc0) && (val < 0xff) ) {
		input_event(input, EV_KEY, KEY_1, 1);
	} else {
		input_event(input, EV_KEY, KEY_1, 0);
	}
	input_sync(input);
}
...
static int ioaccel_probe(struct i2c_client *client)
{
	...
	input_setup_polling(ioaccel->input, ioaccel_poll);
	input_set_poll_interval(ioaccel->input, 50);
	...
	input_set_drvdata(ioaccel->input, ioaccel);
	return input_register_device(ioaccel->input);
}

static void ioaccel_remove(struct i2c_client *client)
{
	struct ioaccel_dev *ioaccel;

	ioaccel = i2c_get_clientdata(client);
	input_unregister_device(ioaccel->input);
